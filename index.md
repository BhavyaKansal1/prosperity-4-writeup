...
**Key insight:** The two products required entirely different strategies, and figuring out *which is which* was 80% of the work. ASH was textbook mean-reversion with a wide 16-tick spread, autocorrelation of -0.49, true price anchored at 10,000. PEPPER was secretly trending with a tiny drift of 0.001 per timestamp that compounded to ~1,000 XIRECs per day if you held the position limit.
...
**What anyone could have figured out:** Plot the prices. Compute autocorrelation. ASH bounces; PEPPER drifts. The hard part wasn't seeing the patterns but committing capital to the trending one. Most teams default to "neutral market-making" because it feels safer, but it leaves PEPPER's deterministic alpha on the table.
...
- Built PEPPER position more aggressively, and extended the startup phase to 30,000 timestamps, increased cross-edge from 3.0 to 8.0. Added a `max_book_fair_premium` cap to avoid overpaying during volatile book moments.
...
Thanks IMC Trading for running Prosperity every year and keeping it accessible to students. The educational value of this competition is hard to overstate.
